  ...
  return (
    ...
    >
      ...
      <div className={`bg-gradient-to-r ${tier.gradient} p-[1px]`}>
        <div className="bg-zinc-900 rounded-t-[15px]">
          <div className="flex items-center justify-between px-4 pt-4 pb-3">
            ...
            <div className="text-right">
              ...
              <p className="text-sm font-black text-zinc-50">{data.daysLeft} ngày</p>
            </div>
          </div>
          ...
          <div className="px-4 pb-3 flex items-center gap-3">
            {data.isPromotionZone ? (
              <div className="flex items-center gap-1 text-[11px] font-bold text-emerald-400 bg-emerald-950/50 border border-emerald-200 dark:border-emerald-700/40 px-2.5 py-1 rounded-full">
                <TrendingUp size={11} /> Vùng thăng hạng
              </div>
            ) : data.isRelegationZone ? (
              <div className="flex items-center gap-1 text-[11px] font-bold text-red-400 bg-red-950/50 border border-red-200 dark:border-red-700/40 px-2.5 py-1 rounded-full">
                <TrendingDown size={11} /> Vùng xuống hạng
              </div>
            ) : (
              <div className="flex items-center gap-1 text-[11px] font-bold text-zinc-400 bg-zinc-800/50 border border-zinc-700/40 px-2.5 py-1 rounded-full">
      ...
      <div className="px-4 py-2 space-y-1">
        <p className="text-[10px] font-bold text-zinc-400 uppercase tracking-wider mb-2">Top 5</p>
        <AnimatePresence>
          {topMembers.map((m, i) => {
            const isMe = m.isMe;
            ...
            return (
              <motion.div
                ...
                className={`flex items-center gap-2.5 px-3 py-2 rounded-xl transition-colors border border-l-[3px] border-l-emerald-500 dark:border-l-emerald-500 ${
                  isMe
                    ? "bg-emerald-950/40 border-y-emerald-700/40 border-r-emerald-700/40"
                    : "bg-zinc-800/40 border-y-zinc-700/20 border-r-zinc-700/20"
                }`}
              >
                <span className="text-sm w-5 text-center shrink-0">
                  {rankEmoji ?? <span className="text-[11px] font-bold text-zinc-500">#{m.rank}</span>}
                </span>
                <p className={`flex-1 text-sm font-semibold truncate ${isMe ? "text-emerald-300" : "text-zinc-300"}`}>
                  {isMe ? "Bạn" : m.display_name}
                </p>
                <p className={`text-xs font-black tabular-nums ${isMe ? "text-emerald-400" : "text-zinc-400"}`}>
                  {m.xp_this_week} XP
                </p>
              </motion.div>
            );
        ...
        {myEntry && data.myRank > 5 && (
          <>
            ...
            <div className="flex items-center gap-2.5 px-3 py-2 rounded-xl border border-l-[3px] border-l-zinc-600 bg-emerald-950/40 border-y-emerald-700/40 border-r-emerald-700/40">
              <span className="text-[11px] font-bold text-zinc-500 w-5 text-center shrink-0">#{data.myRank}</span>
              <p className="flex-1 text-sm font-semibold text-emerald-300 truncate">Bạn</p>
              <p className="text-xs font-black tabular-nums text-emerald-400">{data.myXp} XP</p>
            </div>
          </>
        )}
      </div>
      ...
      <div className="px-4 pb-4 pt-2">
        <Link
          ...
          className="flex items-center justify-center gap-1.5 w-full py-2 rounded-xl bg-zinc-900 hover:bg-zinc-800 text-zinc-400 hover:text-zinc-200 text-xs font-bold transition-colors border border-zinc-700/50"
        >
          <Trophy size={12} />
...
function LeagueCardSkeleton() {
  return (
    <div className="bg-white/5 border border-white/10 rounded-2xl p-4 space-y-3 animate-pulse backdrop-blur-xl">
      <div className="flex items-center gap-3">
      ...
      {[0, 1, 2].map((i) => (
        <div key={i} className="h-9 bg-zinc-800/60 rounded-xl" />
      ))}
    </div>
  );
}

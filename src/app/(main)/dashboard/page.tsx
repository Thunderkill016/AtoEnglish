...
export default function DashboardPage() {
  ...
  const dailyQuests = [
    ...
    { id: 2, text: "Ôn tập 10 thẻ từ vựng SRS", xp: "+15 XP", completed: false },
    ...
    <div className="relative mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8 space-y-8">
      ...
      <div className="grid gap-8 lg:grid-cols-3 items-start">
        ...
        <div className="lg:col-span-2 space-y-8">
          ...
          <div className="group rounded-3xl border border-glass bg-glass p-6 sm:p-8 shadow-[0_8px_30px_rgb(0,0,0,0.015)] hover:shadow-primary/5 hover:border-primary/20 transition-all duration-300 flex flex-col justify-between min-h-[300px] relative overflow-hidden">
            <div className="absolute top-0 right-0 p-8 opacity-[0.03] text-primary">
              <BookOpen className="size-44" />
            </div>
            ...
            <div className="space-y-5 relative z-10">
              <div className="flex items-center justify-between">
                ...
                <span className="inline-flex items-center gap-1 text-[10px] font-bold px-2.5 py-0.5 rounded-full bg-primary/10 text-primary border border-primary/25">
                  Pha 2: Processing
                </span>
              </div>
              ...
              <div className="flex gap-2 pt-1">
                <span className="text-[10px] bg-muted/65 text-muted-foreground px-2.5 py-1 rounded-xl border border-border/40 font-bold uppercase tracking-wider">Từ vựng (+10)</span>
                <span className="text-[10px] bg-muted/65 text-muted-foreground px-2.5 py-1 rounded-xl border border-border/40 font-bold uppercase tracking-wider">Nghe hiểu</span>
              </div>
            </div>

            <div className="mt-8 pt-4 border-t border-border/40 relative z-10 flex justify-end">
              <Link href="/learn">
                <Button className="h-10 px-5 bg-primary hover:bg-primary/95 text-primary-foreground font-semibold rounded-xl flex items-center gap-2 group/btn transition-all duration-300 shadow-md shadow-primary/5">
        ...
        {/* RIGHT COLUMN: Circular Stats, Streak, Daily Quests & SRS (1/3 width) */}
        <div className="space-y-8">
          ...
          <div className="rounded-3xl border border-glass bg-glass p-6 shadow-[0_8px_30px_rgb(0,0,0,0.015)] flex flex-col items-center justify-center text-center space-y-4">
            <h3 className="font-bold text-xs text-muted-foreground uppercase tracking-wider w-full text-left">
              Tiến trình XP Ngày
            </h3>
          ...
          <div className="rounded-3xl border border-glass bg-glass p-6 shadow-[0_8px_30px_rgb(0,0,0,0.015)] flex items-center justify-between gap-4">
            ...
            {/* Soft circle bg for flame icon instead of rounded square */}
            <span className="flex size-11 items-center justify-center rounded-full bg-orange-500/10 text-orange-500 animate-flame shrink-0">
              <Flame className="size-5.5 fill-orange-500" />
            </span>
          </div>

          {/* Daily Quests Card (Moved here to balance grid height) */}
          <div className="rounded-3xl border border-glass bg-glass p-6 shadow-[0_8px_30px_rgb(0,0,0,0.015)]">
            <div className="flex items-center justify-between mb-4 pb-2 border-b border-border/30">
              <span className="text-xs font-bold text-muted-foreground uppercase tracking-wider">Nhiệm vụ hôm nay</span>
              <span className="text-[9px] font-bold px-2 py-0.5 rounded-md bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border border-emerald-500/10">
                1 / 3
              </span>
            </div>
            ...
            <div className="space-y-3">
              {dailyQuests.map((quest) => (
                <div key={quest.id} className="flex items-start gap-3 text-xs py-1 rounded-lg">
                  <span className="shrink-0 text-primary mt-0.5">
                    {quest.completed ? (
                      <CheckCircle2 className="size-4.5 fill-primary text-primary-foreground" />
                    ) : (
                      <Circle className="size-4.5 text-muted-foreground hover:text-primary transition-colors cursor-pointer" />
                    )}
                  </span>
                  ...
                  <span className="text-[10px] font-bold font-mono text-muted-foreground shrink-0 bg-muted/65 px-1.5 py-0.5 rounded border border-border/40">{quest.xp}</span>
                </div>
              ))}
            </div>
          </div>

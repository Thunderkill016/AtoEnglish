  ...
  return (
    <div className="relative mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8 space-y-8">
      ...
      <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
        {stats.map((stat, idx) => {
          ...
          return (
            <div
              key={idx}
              className="group relative overflow-hidden rounded-3xl bg-glass border border-glass p-5 shadow-[0_8px_30px_rgb(0,0,0,0.01)] hover:shadow-primary/5 hover:border-primary/20 transition-all duration-300"
            >
              ...
              <div className="mt-3">
                ...
                <p className="text-[11px] text-muted-foreground mt-0.5 font-medium">{stat.sub}</p>
              </div>
            </div>
          );
        })}
      ...
      <div className="grid gap-8 lg:grid-cols-3">
        {/* Weekly Chart */}
        <div className="lg:col-span-2 rounded-3xl border border-glass bg-glass p-6 sm:p-8 space-y-6 shadow-[0_8px_30px_rgb(0,0,0,0.01)]">
          <div className="flex items-center justify-between">
            <div className="space-y-1">
              <h3 className="font-bold text-sm text-foreground uppercase tracking-wider">Kinh nghiệm hàng ngày</h3>
              <p className="text-xs text-muted-foreground">Lượng XP thu nhận được trong 7 ngày qua</p>
            </div>
            <span className="inline-flex items-center gap-1 text-xs font-semibold text-muted-foreground bg-muted/65 px-2.5 py-1 rounded-xl border border-border/40">
          ...
          <div className="h-56 flex items-end justify-between gap-2 px-2 pt-6 border-b border-border/40">
            {weeklyData.map((data, idx) => (
              <div key={idx} className="flex-1 flex flex-col items-center gap-2 group h-full justify-end">
                ...
                {/* Bar */}
                <div
                  className={`w-full max-w-[24px] rounded-t-md bg-gradient-to-t from-primary/80 to-primary group-hover:from-primary group-hover:to-emerald-400 transition-all duration-300 ${data.height}`}
        ...
        <div className="rounded-3xl border border-glass bg-glass p-6 sm:p-8 space-y-6 shadow-[0_8px_30px_rgb(0,0,0,0.01)] flex flex-col justify-between">
          <div className="space-y-4">
            <h3 className="font-bold text-sm text-foreground uppercase tracking-wider">Hộp bộ nhớ SRS</h3>
            ...
            <div className="space-y-3 pt-2">
              {[
                { name: "Hộp 1 (Mới học)", count: 18, pct: 15, color: "bg-red-500" },
                { name: "Hộp 2 (Bắt đầu nhớ)", count: 32, pct: 27, color: "bg-orange-500" },
                { name: "Hộp 3 (Nhớ tốt)", count: 50, pct: 41, color: "bg-blue-500" },
                { name: "Hộp 4 (Trí nhớ dài hạn)", count: 20, pct: 17, color: "bg-emerald-500" },
              ].map((box, idx) => (
                <div key={idx} className="space-y-1">
                  <div className="flex justify-between text-[10px] font-semibold">
                    <span className="text-foreground">{box.name}</span>
                    <span className="text-muted-foreground">{box.count} từ ({box.pct}%)</span>
                  </div>
        ...
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
          {achievements.map((ach, idx) => {
            ...
            const tierBorder = 
              ach.unlocked
                ? ach.tier === "gold"
                  ...
                  : ach.tier === "silver"
                  ? "border-slate-400/20 bg-slate-400/5 hover:border-slate-400/40"
                  : "border-amber-700/20 bg-amber-700/5 hover:border-amber-700/40"
            ...
            return (
              <div
                key={idx}
                className={`group rounded-3xl border p-5 flex flex-col justify-between transition-all duration-300 shadow-[0_4px_25px_rgba(0,0,0,0.005)] ${tierBorder}`}
              >
                <div className="space-y-3">
                  <div className="flex justify-between items-start">
                    <span className={`flex size-9 items-center justify-center rounded-xl ${tierIconColor}`}>
                      {ach.unlocked ? <Icon className="size-4.5" /> : <Lock className="size-4" />}
                    </span>
                    {ach.unlocked ? (

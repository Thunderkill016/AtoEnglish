...
export default function RoadmapClient({ userCefrLevel, unitCountByLevel, completedByLevel, nextUnitRoute }: RoadmapClientProps) {
  ...
  const userLevelIndex = CEFR_ORDER.indexOf(userCefrLevel as typeof CEFR_ORDER[number]);
  ...
  const overallPercent = Math.round(
    levels.reduce((sum, l) => sum + l.completedPercent, 0) / levels.length
  );
    ...
    <div className="relative mx-auto max-w-6xl px-4 py-6 sm:py-8 sm:px-6 lg:px-8 space-y-6 sm:space-y-8 bg-grid-pattern min-h-screen overflow-x-hidden">
      ...
      >
        ...
        <div className="shrink-0 flex items-center gap-2 rounded-2xl bg-glass border border-glass px-4.5 py-3 text-xs sm:text-sm font-bold text-foreground shadow-[0_8px_30px_rgb(0,0,0,0.01)]">
          ...
          <span>Tổng tiến độ: {overallPercent}% hoàn thành</span>
        </div>
      </motion.div>
      ...
      <div className="relative space-y-8 sm:space-y-16 py-6 sm:py-8">
        ...
        <motion.div
          initial={{ height: 0 }}
          animate={{ height: `${Math.min(overallPercent, 100)}%` }}
            ...
            >
              ...
              <motion.div
                ...
                className={`w-full lg:w-[46%] rounded-3xl border p-6 sm:p-7 bg-glass border-glass shadow-[0_8px_30px_rgb(0,0,0,0.015)] ${
                  isActive ? "ring-1 ring-primary/35 border-primary/20 shadow-[0_8px_35px_oklch(var(--primary)/0.03)]" : ""
                }`}
              >

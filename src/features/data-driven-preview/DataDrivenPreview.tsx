...
export function DataDrivenPreview() {
  ...
  const resetAttempt = () => { setAnswer(""); setAnswerSource(null); setFeedback(null); };
  ...
  const evaluate = () => {
    ...
    const channel: EvidenceKey | null =
      action.kind === "retrieve" ? "retrieval" :
      action.kind === "produce" ? "production" :
      action.kind === "repair" || action.kind === "retry" ? "repair" :
      action.kind === "transfer" ? "transfer" : null;

    if (channel && answerSource === "speech" && ok) setEvidence((current) => ({ ...current, [channel]: true }));
        ...
        <div className="mx-auto w-full max-w-5xl px-4 py-8 sm:px-6 sm:py-14">
          ...
          <section className="grid gap-5 lg:grid-cols-[1.4fr_.8fr]">
            ...
            <div className="rounded-[32px] border border-black/10 bg-white p-6 sm:p-8">
              ...
              <p className="mt-2 text-sm leading-6 text-black/55">Gate: retrieval-before-reveal, real speech path, feedback/repair/retry, changed-context transfer, delayed review và evidence trace.</p>
        ...
        <section className="flex flex-1 flex-col justify-center pb-12">
          ...
          <div className="mt-8 rounded-[28px] border border-black/10 bg-white p-5 shadow-[0_24px_80px_rgba(20,30,20,.07)] sm:p-8">
            ...
            {action.kind === "comprehend" ? (
              <div className="grid gap-3 sm:grid-cols-3">{["name", "job", "country"].map((choice) => <button key={choice} type="button" onClick={() => { setAnswer(choice); setAnswerSource("text"); setFeedback(null); }} className={`rounded-2xl border px-4 py-4 text-left font-semibold ${answer === choice ? "border-[#2d6a4f] bg-[#edf5ef]" : "border-black/10"}`}>{choice}</button>)}</div>
            ) : action.modality === "speech" ? (
              <>
                <div className="flex flex-wrap gap-3"><button type="button" onClick={startSpeech} disabled={listening} className="flex items-center gap-2 rounded-full bg-[#171713] px-5 py-3 text-sm font-semibold text-white disabled:opacity-40"><Mic2 className="size-4" /> {listening ? "Đang nghe…" : "Nói bằng mic"}</button>{!speechAvailable && <span className="flex items-center gap-2 text-xs text-[#8a5b00]"><CircleAlert className="size-4" /> Browser không hỗ trợ speech recognition</span>}</div>
                <textarea value={answer} onChange={(event) => { setAnswer(event.target.value); setAnswerSource("text"); setFeedback(null); }} placeholder="Transcript hoặc text fallback…" className="mt-4 min-h-24 w-full resize-none rounded-2xl border border-black/10 bg-[#faf9f5] p-4 outline-none focus:border-[#2d6a4f]/60" />
                <div className="mt-3 flex flex-wrap gap-2"><button type="button" onClick={evaluate} disabled={!answer || answerSource !== "speech"} className="rounded-full bg-[#2d6a4f] px-4 py-2 text-xs font-bold text-white disabled:opacity-30">Đánh giá speech transcript</button><button type="button" onClick={evaluate} disabled={!answer} className="rounded-full border border-black/10 px-4 py-2 text-xs font-bold text-black/55 disabled:opacity-30">Text fallback</button></div>
              </>
            ) : null}

            {action.kind === "comprehend" && <button type="button" onClick={evaluate} disabled={!answer} className="mt-4 rounded-full bg-[#171713] px-5 py-2.5 text-sm font-semibold text-white disabled:opacity-30">Kiểm tra</button>}
          ...
          <div className="mt-8 flex items-center justify-between"><button type="button" onClick={() => { setStep((value) => Math.max(0, value - 1)); resetAttempt(); }} disabled={step === 0} className="rounded-full px-4 py-3 text-sm font-semibold text-black/45 disabled:opacity-20"><RefreshCw className="mr-2 inline size-3" />Quay lại</button><button type="button" onClick={next} className="flex items-center gap-2 rounded-full bg-[#2d6a4f] px-6 py-3 text-sm font-semibold text-white">{step === lesson.actions.length - 1 ? "Kết thúc slice" : "Tiếp theo"}<ArrowRight className="size-4" /></button></div>
        </section>

        <footer className="pb-4 text-center text-[11px] leading-5 text-black/35"><Headphones className="mr-1 inline size-3" /> Transcript feedback ≠ pronunciation score. Text fallback ≠ speaking evidence.{supportUsed && " Vietnamese support usage is tracked separately."}</footer>
      </div>
    </main>
  );
}

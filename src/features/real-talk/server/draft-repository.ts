...
import type {
  AtomicPrivateDraftRpcClient,
  AtomicPrivateDraftRpcRow,
...
export type PersistPrivateDraftResult =
  | {
      ...
      persistedIds: AtomicPrivateDraftRpcRow;
    }
  | GenerationFailure;
...
export async function persistOwnerPrivateDraft(params: {
  ...
  try {
    ...
    const rpcClient = supabase as unknown as AtomicPrivateDraftRpcClient;
    ...
    const persistedIds = data?.[0];

    if (error || !persistedIds?.video_id || !persistedIds.lesson_id) {

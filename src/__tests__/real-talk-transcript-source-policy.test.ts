...
describe("Real Talk transcript source policy", () => {
  it("defaults to approved-only public content", () => {
    expect(resolveTranscriptSourcePolicy({ NODE_ENV: "development" })).toEqual({
      runtime: "non_production",
      useCase: "public_content",
      experimentalFlagEnabled: false,
      privateProductionFlagEnabled: false,
      mode: "approved_only",
    });
  });

  it("allows an approved source with matching human provenance in production", async () => {
  ...
  it("rejects approved metadata without provenance", async () => {
  ...
  it("blocks experimental sources outside production unless explicitly enabled", async () => {
  ...
  it("allows experimental sources for non-production private drafts with the dev flag", async () => {
    const { adapter, acquire } = createAdapter("experimental");
  ...
  it("allows experimental production only for private drafts and only with the separate flag", async () => {
    const { adapter, acquire } = createAdapter("experimental");
    ...
    await expect(
      acquireTranscriptForCompilation({
        ...
        environment: {
          NODE_ENV: "production",
          REAL_TALK_ALLOW_PRIVATE_PRODUCTION_TRANSCRIPTS: "true",
        },
      }),
    ).resolves.toMatchObject({ cues });
    expect(acquire).toHaveBeenCalledOnce();
  ...
  it("never allows an experimental source for public content", async () => {
    const { adapter, acquire } = createAdapter("experimental");
  ...
  it("keeps the server action free of unofficial package imports", () => {
    ...
    expect(adapterSource).not.toContain("from \"youtube-transcript\"");

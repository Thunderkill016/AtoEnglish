...
- **Constraints**: Pure deterministic functions; no ambient time (`Date.now()`); zero runtime, database, UI, network, or provider changes.
- **Scope**: Canonical construct keys, evidence event validation, sufficient statistics accumulation, uncertainty/sufficiency classification, deterministic replay, and reducer equivalence.
...
- **Evidence Integrity and Scoped Authority**: PASS. Enforces observation != evidence != state != mastery. Explicitly scopes uncalibrated outputs to `decisionScope: "routing-only"`.

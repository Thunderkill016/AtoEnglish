...
-- PostgREST roles need table privileges before row-level policies can decide
-- which rows are visible or mutable. Anonymous access is read-only; the next
-- migration limits it to public rows. Authenticated users receive DML grants,
-- with the next migration enforcing owner-private semantics.

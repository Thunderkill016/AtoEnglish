...
const speakingPath = "src/components/learn/sections/SpeakingSection.tsx";
let speaking = fs.readFileSync(speakingPath, "utf8");

speaking = replaceOnce(
  speaking,
...
speaking = replaceOnce(
  speaking,
`            {level2Recording ? "Dừng" : level2Transcript ? "Thử lại" : "Bắt đầu nói"}
`,
`            {level2Recording
              ? "Dừng"
              : unit.unitId === "unit-1"
...
fs.writeFileSync(speakingPath, speaking);

const unitPath = "src/lib/data/units/unit1.ts";
